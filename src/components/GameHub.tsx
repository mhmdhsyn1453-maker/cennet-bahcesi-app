...
// ==========================================
const PassaparolaGame: React.FC<{ isDarkMode: boolean }> = ({ isDarkMode }) => {
  ...
  const handleStart = () => {
    ...
    TURKISH_LETTERS.forEach(l => {
      qMap[l] = getRandomQuestionForLetter(l, 'all');
    });
  ...
  const proceedToNext = (updatedStates: Record<string, 'pending' | 'correct' | 'incorrect' | 'passed'>) => {
    ...
    if (round === 1) {
      // 1. Turda sıradaki harfe git
      const nextIdx = currentIdx + 1;
      if (nextIdx < TURKISH_LETTERS.length) {
        setCurrentIdx(nextIdx);
      } else {
        // 1. Tur bitti, pas geçilenler var mı kontrol et
        const passes = TURKISH_LETTERS.map((l, i) => ({ l, i })).filter(item => updatedStates[item.l] === 'passed').map(item => item.i);
        if (passes.length > 0) {
          setRound(2);
          setPassHistory(passes);
          setCurrentIdx(passes[0]);
        } else {
          playSound('complete');
          setGameStatus('ended');
        }
      ...
      if (remainingPasses.length > 0) {
        ...
        let nextPassIdx = -1;
        // Mevcut pas indeksinden sonraki pası bulmaya çalış
        for (let i = currentPassIndexInHistory + 1; i < passHistory.length; i++) {
          if (updatedStates[TURKISH_LETTERS[passHistory[i]]] === 'passed') {
            nextPassIdx = passHistory[i];
            break;
          }
        }
        // Eğer sonrakiler bitmişse başa dönüp ilk pası al
        if (nextPassIdx === -1) {
          nextPassIdx = remainingPasses[0];
        }
        setCurrentIdx(nextPassIdx);
      } else {
        // Tüm paslar cevaplandı, oyun biter
        playSound('complete');
        setGameStatus('ended');
      }
    }
              ...
              {TURKISH_LETTERS.map((letter, i) => {
                ...
                return (
                  <div
                    ...
                    className={`w-9 h-9 rounded-full border-2 flex items-center justify-center font-display font-black text-xs select-none transition-all duration-300 shadow ${colorClass}`}
                  >
                    {letter}
                  </div>
                );
          ...
          <div className="lg:col-span-6 flex flex-col gap-4">
            ...
            <div className="bg-white dark:bg-slate-800 border-3 border-slate-200 dark:border-slate-700 p-5 rounded-[2rem] shadow-md flex flex-col gap-3 min-h-[140px] justify-center relative">
              ...
              <div className="flex gap-2 mt-1">
                {activeQuestionObj.clues.map((clue, index) => (
                  <span key={index} className="text-[10px] font-bold text-slate-400 dark:text-slate-500 bg-slate-50 dark:bg-slate-900/50 px-2 py-0.5 rounded-md">
                    💡 İpucu: {clue}
                  </span>
                ))}
              </div>
            ...
            <div className="flex flex-col gap-2">
              ...
              <div className="flex justify-between items-center bg-slate-50 dark:bg-slate-900/40 border border-slate-200 dark:border-slate-800 p-2.5 rounded-xl">
                ...
                >
                  {showReveal ? 'Cevabı Gizle 👁️' : 'Cevabı Gör 👀'}
                </button>
